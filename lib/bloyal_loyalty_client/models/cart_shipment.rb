  ...
  class CartShipment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BloyalLoyaltyClient::CartShipment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BloyalLoyaltyClient::CartShipment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [uid, number, type, recipient, address_uid, address_external_id, carrier_uid, carrier_external_id, carrier_code, service_uid, service_external_id, service_code, location_uid, location_external_id, location_code, fulfillment_house, externally_applied_charge, charge, charge_override, externally_applied_discount, discount, discount_reason_code, discount_reason_name, discount_details, gift_package, gift_comment, estimated_ship_date, ship_date, instructions, externally_applied_tax, tax_details].hash
    end
