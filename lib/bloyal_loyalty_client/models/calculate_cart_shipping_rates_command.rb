...
module BloyalLoyaltyClient
  class CalculateCartShippingRatesCommand
    attr_accessor :cart
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BloyalLoyaltyClient::CalculateCartShippingRatesCommand` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BloyalLoyaltyClient::CalculateCartShippingRatesCommand`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'shipment_uids')
        if (value = attributes[:'shipment_uids']).is_a?(Array)
          self.shipment_uids = value
        end
      end
    ...
    def hash
      [cart, cart_uid, cart_external_id, shipment_uids, order_processor, device_uid, store_code, device_code, cashier_uid, cashier_code, cashier_external_id, uid, reference_number, system_uid, connector_uid].hash
    end
