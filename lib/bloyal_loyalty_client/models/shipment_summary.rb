  ...
  class ShipmentSummary
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BloyalLoyaltyClient::ShipmentSummary` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BloyalLoyaltyClient::ShipmentSummary`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      shipment_type_validator = EnumAttributeValidator.new('Object', ['Unknown', 'Shipment', 'Pickup', 'EGift', 'ShipToLocation', 'CarryOut'])
    ...
    def hash
      [uid, external_id, address, price, weight, location_uid, location_code, shipment_type].hash
    end
