  ...
  class UpdateCustomerReference
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BloyalLoyaltyClient::UpdateCustomerReference` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BloyalLoyaltyClient::UpdateCustomerReference`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      status_validator = EnumAttributeValidator.new('Object', ['Pending', 'Succeeded', 'Failed', 'Reversed', 'Processing'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Object', ['Pending', 'Succeeded', 'Failed', 'Reversed', 'Processing'])
    ...
    def hash
      [customer_uid, external_id, verified, uid, created, status, status_message, retry_count, system_uid, connector_uid, device_uid, store_code, device_code, cashier_uid, cashier_code, cashier_external_id, reference_number].hash
    end
