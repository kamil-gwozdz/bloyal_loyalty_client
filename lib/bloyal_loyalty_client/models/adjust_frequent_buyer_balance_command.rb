...
module BloyalLoyaltyClient
  class AdjustFrequentBuyerBalanceCommand
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BloyalLoyaltyClient::AdjustFrequentBuyerBalanceCommand` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BloyalLoyaltyClient::AdjustFrequentBuyerBalanceCommand`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'rule_uid')
        self.rule_uid = attributes[:'rule_uid']
      end
    ...
    def hash
      [customer_uid, customer_external_id, customer_source_external_id, customer_code, rule_uid, rule_code, quantity, comment, created_local, created, device_uid, store_code, device_code, cashier_uid, cashier_code, cashier_external_id, uid, reference_number, system_uid, connector_uid].hash
    end
