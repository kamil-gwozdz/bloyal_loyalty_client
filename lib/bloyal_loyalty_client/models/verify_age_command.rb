  ...
  class VerifyAgeCommand
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BloyalLoyaltyClient::VerifyAgeCommand` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BloyalLoyaltyClient::VerifyAgeCommand`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'cashier_name')
        self.cashier_name = attributes[:'cashier_name']
      end
    ...
    def valid?
      method_validator = EnumAttributeValidator.new('Object', ['Unknown', 'InPerson', 'OnFile', 'EstablishedMember', 'External', 'Employee', 'Other'])
    ...
    def method=(method)
      validator = EnumAttributeValidator.new('Object', ['Unknown', 'InPerson', 'OnFile', 'EstablishedMember', 'External', 'Employee', 'Other'])
    ...
    def hash
      [customer_uid, customer_external_id, customer_code, cashier_name, created, created_local, method, device_uid, store_code, device_code, cashier_uid, cashier_code, cashier_external_id, uid, reference_number, system_uid, connector_uid].hash
    end
