...
module BloyalLoyaltyClient
  class RenewClubMembershipCommand
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BloyalLoyaltyClient::RenewClubMembershipCommand` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BloyalLoyaltyClient::RenewClubMembershipCommand`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [membership_uid, customer_uid, club_uid, expiration_date, transaction_uid, transaction_external_id, referral_uid, sponsor_uid, fulfillment, payment, auto_billing_day, auto_billing_start_date, member_comment, product_preference_type, product_preferences, device_uid, store_code, device_code, cashier_uid, cashier_code, cashier_external_id, uid, reference_number, system_uid, connector_uid].hash
    end
