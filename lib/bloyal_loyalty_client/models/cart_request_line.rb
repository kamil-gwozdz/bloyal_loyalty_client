  ...
  class CartRequestLine
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BloyalLoyaltyClient::CartRequestLine` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BloyalLoyaltyClient::CartRequestLine`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [uid, external_id, product_uid, product_code, product_external_id, product_name, category_uid, category_code, category_external_id, department_uid, department_code, department_external_id, sub_category1_uid, sub_category1_code, sub_category1_external_id, sub_category2_uid, sub_category2_code, sub_category2_external_id, sub_category3_uid, sub_category3_code, sub_category3_external_id, sub_category4_uid, sub_category4_code, sub_category4_external_id, sales_rep_code, shipment_uid, shipment_number, quantity, full_price, price, externally_applied_sale_price, sale_price, sale_price_reason_code, sale_price_reason_name, externally_applied_discount, discount, discount_reason_code, discount_reason_name, net_price, cost, replacement_cost, last_cost, return_reason_code, description, comment, weight, tax_exempt, externally_applied_tax, tax_details, price_source, gift_card_number, gift_card_amount, serial_number, line_options].hash
    end
