  ...
  class CreditCardMethod
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BloyalLoyaltyClient::CreditCardMethod` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BloyalLoyaltyClient::CreditCardMethod`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'card_mask')
        self.card_mask = attributes[:'card_mask']
      end
    ...
    def valid?
      ...
      problem_validator = EnumAttributeValidator.new('Object', ['Unknown', 'Expired', 'Invalid'])
      return false unless problem_validator.valid?(@problem)
      true
    end
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] problem Object to be assigned
    def problem=(problem)
      validator = EnumAttributeValidator.new('Object', ['Unknown', 'Expired', 'Invalid'])
      unless validator.valid?(problem)
        fail ArgumentError, "invalid value for \"problem\", must be one of #{validator.allowable_values}."
      end
      @problem = problem
    end
    ...
    def hash
      [card_mask, card_type, card_token, problem, expiration, billing_name, billing_address1, billing_address2, billing_city, billing_state, billing_postal_code, billing_country].hash
    end
