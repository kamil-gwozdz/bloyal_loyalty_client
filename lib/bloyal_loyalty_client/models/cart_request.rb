  ...
  class CartRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BloyalLoyaltyClient::CartRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BloyalLoyaltyClient::CartRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [uid, external_id, source_external_id, order_uid, order_external_id, channel_code, cashier_code, price_level_code, guest_checkout, comment, reference_number, loyalty_reference_number, title, batch_title, custom_code1, custom_code2, custom_code3, custom_code4, custom_code5, custom_code6, custom_code7, custom_code8, customer, referred_by, sponsor, shipments, lines, accounts, payments, created, created_local, externally_applied_discount, discount, discount_reason_code, discount_reason_name, return_uid, return_external_id, return_source_external_id].hash
    end
